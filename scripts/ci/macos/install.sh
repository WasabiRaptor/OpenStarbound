#!/bin/sh -e
STEAM_LIBPATHS_CONFIG="${HOME}/Library/Application Support/Steam/config/libraryfolders.vdf"
...
if [ -z "${STARBOUND_INSTALL_PATH}" ]; then
	...
	if [ $DID_FIND_STARBOUND == false ]; then
		...
		echo $'\t' "sh" "${0}" "/full/path/to/starbound/root/dir"
		exit 1
	fi
fi
...
if [ ! -d "${STARBOUND_INSTALL_PATH}/osx/Starbound.app" ]; then
